...
const getVerificDr = async (data) => {
  ...
  const query = "Select nc.*, pl.*  from  nc_processed_content nc , nc_process_logs pl where pl.id in (select id from nc_process_logs where processor = $1 and status = 'DRAFT_VERIFY' ORDER BY  nc_date_created ASC limit 1) and pl.status = 'DRAFT_VERIFY' and nc.plid = (select id from nc_process_logs where processor = $1 and status = 'DRAFT_VERIFY' ORDER BY  nc_date_created ASC limit 1)"
...
const getDrafts = async (data) => {
  const { process_name } = data;
  const query = "Select nc.*, pl.*  from  nc_processed_content nc , nc_process_logs pl where pl.id in (select id from nc_process_logs where processor = $1 and status = 'DRAFT_START' ORDER BY  nc_date_created ASC limit 1) and pl.status = 'DRAFT_START' and nc.plid = (select id from nc_process_logs where processor = $1 and status = 'DRAFT_START' ORDER BY  nc_date_created ASC limit 1) ";
  try {
...
//  const query = "UPDATE nc_articles SET nc_isdraft = 'DRAFT_INFO' WHERE id = $1 and nc_isdraft = 'DRAFT_START'   AND  $1 = ( SELECT * from nc_articles where id = $1 )     RETURNING *";

const query = "SELECT * from nc_articles where id = $1 ";
  ...
  console.log("@@ getContentHtml =========id :" ,id );
      ...
      try {
          ...
          if (result.rows.length > 0) {
              ...
              console.log("getContentHtml - getContentHtml -result.rows[0].results_3====================");
              return result.rows;       
...
//Select nc.*, pl.* from nc_processed_content nc, nc_process_logs pl  where pl.id in (select id from nc_process_logs where processor =$1) and pl.status = 'DRAFT_VERIFY_2' and nc.plid in (select id from nc_process_logs where processor =$1) ORDER BY  pl.nc_date_created ASC limit 1
...
const getVerificDr2 = async (data) => {
  ...
  const { process_name } = data;
  const query = "Select nc.*, pl.* from nc_processed_content nc, nc_process_logs pl  where pl.id in (select id from nc_process_logs where processor =$1 and status = 'DRAFT_VERIFY_2' ) and pl.status = 'DRAFT_VERIFY_2' and nc.plid = pl.id ORDER BY  pl.nc_date_created ASC limit 1";
