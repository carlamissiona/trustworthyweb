...
app.get("/drafts_final", async(req, res) => {
    let html_final = "";
// if there is results3 in drafts 
    try {
...
html_final = nc_content;
                        for (let i = 0; i < cited_collectn.length; i++) {

                          let nums = [cited_collectn[i].indexOf("Mistral-Small-24B") , cited_collectn[i].indexOf("Llama-3.1-8B") , cited_collectn[i].indexOf("Llama-4-Scout-17B") ]; 
                          console.log("=----------------------------NUMS -------------", nums);

                           const models = [
                                  { llm_in: cited_collectn[i].indexOf("Mistral-Small-24B Supporting Info:") , model_name: "Mistral-Small-24B" },
                                  { llm_in: cited_collectn[i].indexOf("Llama-3.1-8B Supporting Info:"), model_name: "Llama-3.1-8B" },
                                  { llm_in: cited_collectn[i].indexOf("Llama-4-Scout-17B Supporting Info:"), model_name: "Llama-4-Scout-17B" }
                                ];
                            if(models.includes(-1)){
                            continue;
                            }
...
console.log(models);
                    ...
                    console.log("What is the nearest llm ");            console.log(matchedModel.model_name);  // Output: "Trbo"

                                let model_nearest = matchedModel.model_name;
/*
                                console.log("cited_collectn[i].substring(0, cited_collectn[i].indexOf(matchedModel.model_name) )" );

                                console.log(cited_collectn[i].substring(0, cited_collectn[i].indexOf(matchedModel.model_name) ) );
                                console.log(cited_collectn[i].substring(0, cited_collectn[i].indexOf(matchedModel.model_name) ) ); */

                                // get index of the cited 
                                let cit_index = 0; cit_index = nc_content.indexOf( cited_collectn[i].substring(0, cited_collectn[i].indexOf(matchedModel.model_name) ) );
                                // console.log("+++++++++++++++++++++cit_index=");
                                // console.log("+++++++++++++++++++++cit_index=");
                                // console.log(cit_index);

                                // insert html 
                                nums = nums.sort((a, b) => a - b);
                                let cited_aillm1=   cited_collectn[i].substring( nums[0] , nums[1]); 
                                let cited_aillm2=   cited_collectn[i].substring( nums[1] , nums[2]); 
                                let cited_aillm3=   cited_collectn[i].substring( nums[2] ); 

                                console.log( " ai llm ");
                                // console.log("cited_aillm1 ", cited_aillm1);
                                // console.log("cited_aillm1 ", cited_aillm2);
                                // console.log("cited_aillm3 ", cited_aillm3);


                                            let cited_str = cited_collectn[i].substring(0, cited_collectn[i].indexOf(matchedModel.model_name) );
console.log(cited_str, " index of @ html final ",                         html_final.indexOf(cited_str.trim())  );
                    if( html_final.indexOf(cited_str.trim()) == -1 || (cited_str.trim()).length == 0 ){
                                continue;
                            }

                            console.log("==============The Cited ==========", cited_str);
                                // replace cited with spalet 
                                 let rpl_str =" <span style ='font-style: italic;' id='webgraph-" + i + "id'> "; 
                                rpl_str = rpl_str + cited_str.trim() + "</span>";
                                console.log("What is cited with the span >>>>>>>> : " , rpl_str );
                                console.log("What is cited with the span >>>>>>>> : " , rpl_str );
                                console.log("What is cited with the span >>>>>>>> : " , rpl_str );

                                rpl_str = rpl_str +" <span style='display:none;' data-parent ='webgraph-" + i + "id' data-llm ='"+ matchedModel.model_name +"' data-info = 'THE LLM INFO: '"+ cited_aillm1 +"> webgraph-llm </span>";

                                    console.log("rpl_str ==============");
                              console.log(rpl_str);
                              console.log("the str..................... index" ,  (  html_final.indexOf(cited_collectn[i].substring(0, cited_collectn[i].indexOf(matchedModel.model_name) ).trim() )  ) );
                              html_final = html_final.replace( cited_collectn[i].substring(0, cited_collectn[i].indexOf(matchedModel.model_name) ).trim() , rpl_str) ;

                        }

                      }

  console.log("the html  final ++++++++++++++++++++++++++++++++++++++++++++++++++++++!!!!" , html_final);
                }).catch(error);
